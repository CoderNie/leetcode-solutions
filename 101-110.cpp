    ...
    TreeNode* sortedArrayToBSTRecur(vector<int>& nums, int begin, int end) {
        if (begin > end) {
            return NULL;
        } else if (begin == end) {
            ...
            int mid = (end + begin) / 2;
            TreeNode* nowNode = new TreeNode(nums[mid]);
            nowNode->left = sortedArrayToBSTRecur(nums, begin, mid - 1);
            nowNode->right = sortedArrayToBSTRecur(nums, mid + 1, end);
            return nowNode;
        }
    }
    ...
    int isBalancedRecur(TreeNode* root) {
        if (root == NULL)
            return 0;
        int leftHeight = isBalancedRecur(root->left);
        int rightHeight = isBalancedRecur(root->right);
